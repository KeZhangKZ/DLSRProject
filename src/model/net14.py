...
class MYNET14(nn.Module):
    def __init__(self, args, conv=default_conv):
        super(MYNET14, self).__init__()
        feat = args.n_feats
        scale = args.scale[0]
        ...
        # ]
        self.reduce_h = nn.Conv2d(self.n_brmblocks * feat, feat, 1)
        self.conv_h = nn.Conv2d(feat, feat, 3, stride=1, padding=1)

        self.reduce = nn.Conv2d(self.n_brmblocks * feat, feat, 1)
        self.c = nn.Conv2d(feat, feat, 3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        down[-1] = up[-1]
        up[-1] = self.conv_h(self.reduce_h(torch.cat(down, dim=1)))
