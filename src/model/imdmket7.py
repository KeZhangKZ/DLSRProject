...
class IMDMKET7(nn.Module):
    def __init__(self, args, num_modules=6):
        super(IMDMKET7, self).__init__()
        feat = args.n_feats
        scale = args.scale[0]
        ...
        self.shrink = conv_layer(feat, 32, kernel_size=3)
        self.expand = conv_layer(32, feat, kernel_size=3)

        # Transformer
        self.attention = MLABlock(dim=32 * 3 * 3)

        self.tail = pixelshuffle_block(feat, args.n_colors, upscale_factor=scale)
    ...
    def forward(self, input):
        ...
        # b,c,h,w = out_B.shape
        out = self.attention(self.shrink(out_B))

        out_lr = self.expand(out) + out_fea
