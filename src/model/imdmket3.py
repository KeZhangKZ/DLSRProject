...
from model.transformer2 import MLABlock


def make_model(args, parent=False):
    return IMDMKET3(args)
...
class IMDMKET3(nn.Module):
    def __init__(self, args, num_modules=6):
        super(IMDMKET3, self).__init__()
        feat = args.n_feats
        scale = args.scale[0]
    ...
    def forward(self, input):
        ...
        # b,c,h,w = out_B.shape
        in_att = self.reduce(self.LR_conv1(out_B))
        out = self.weight1(self.attention(in_att)) + self.weight2(in_att)
